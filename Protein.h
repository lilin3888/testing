...
public:
	...
	AAProperty(char t[5], char t_1[2], char t_all[15], bool h) {
		strcpy(name, t);
		strcpy(name_1, t_1);
...
public:
	...
	void setParm_charge_radius(const float x, const float y ) {
		fCharge=x;
		fRadius=y;
	}
...
public:
	...
	// set up member
	void setParm_record(const char *str) {
		strcpy(record, str);
	}
...
Atom Atom::operator=(const Atom &pp) //copy an atom object
	...
	bondNo = pp.bondNo;
	for (i = 0; i < 4; i++)
		bondCovalence[i] = pp.bondCovalence[i];
	active = pp.active;
	weight = pp.weight;
...
void Atom::link(Atom &pp) // set a bond pointer to Atom pp
	...
	bond[bondNo++] = &pp;
	if (bondNo > 4) {
		cerr << "Error: more than 4 bonds for atom " << this->No << endl;
		cerr << "it's neighbors are: " << bond[0]->No << " " << bond[1]->No
				<< " " << bond[2]->No << " " << bond[3]->No;
		cerr << endl;
		exit(1);
	}
...
bool Residue::connection(Residue &r2) {
	...
	if (this->mapAtom.count("  C  ") && r2.mapAtom.count("  N  ")) {
		...
		if (*aC - *aN <= bondLengthLimit(*aC, *aN))
			tmp = true;
		else
			tmp = false;
	} else {
...
// output all of the atom record of this residue
	...
	for (int i = 0; (unsigned)i < residue.atom.size(); i++)
		ostrm << residue.atom[i];
	return ostrm;
}
...
// output all of the atom record of this residue
		{
	for (int i = 0; (unsigned)i < chain.residue.size(); i++)
		ostrm << chain.residue[i];
	ostrm << "TER" << endl << endl;
	return ostrm;
}
...
void Protein::readPDB(char *pdbFile) {
	...
	cout << "in readPDB Lin" << endl;
	try {
		ifstream Fin(pdbFile);

		if (!Fin)
			throw strcat("Cannot open PDB file ", pdbFile);
		...
		float x, y, z, fRadiusTmp, fChargeTmp;
		...
		for (;;) {
			...
			if (buf.substr(0, 6) == "ATOM  ")
				newLine = 1;
			else if (buf.substr(0, 6) == "HETATM")
				newLine = 2;
			else if (buf.substr(0, 3) == "TER")
				newLine = 3;
			else if (Fin.eof())
				newLine = 4;
			else
			...
			case 1:
				...
				fChargeTmp = atof(buf.substr(54, 8).c_str());
				fRadiusTmp = atof(buf.substr(62, 7).c_str());

				newAtom.setParm_xyz(x, y, z);
				newAtom.setParm_charge_radius(fChargeTmp, fRadiusTmp);
...
//		cofactor.addAtom(newHetatm);
		...
		for (i = 0; i < int(chain.size()); i++) {
			for (j = 0; j < int(chain[i].residue.size()); j++) {
				for (k = 0; k < int(chain[i].residue[j].atom.size()); k++) {
			...
			if (i != 0) {    // lastChain_p existed
				...
				if (!strcmp(pc->name, lastChain_p->name))
					continue;
			}
...
void Protein::writePDB(char *pdbFile) {
	...
	try {
		...
		for (i = 0; (unsigned)i < chain.size(); i++) {
...
//                order=3: reorder both
	...
	if (!Fout)
		throw strcat("Error: cannot open pdb output file ", file);
	...
	if (chainName[0] == ' ' || chainName[0] == '_') {
		for (i = 0; i < chain.size(); i++) {
			for (j = 0; j < chain[i].atom_p.size(); j++) {
				...
				Fout << "  1.00" << setprecision(2) << setw(6) << atm->TFactor
						<< endl;
			}
			Fout << "TER" << endl;
		}
		...
		if (!mapChain.count(chainName))
			throw strcat("No such a chain name as ", chainName);
...
//  if(bFlag==0){ // bond generation based on distance
	...
	for (i = 0; i < atom_p.size() - 1; i++) {
		for (j = i + 1; j < atom_p.size(); j++) {
			if ((*atom_p[i] - *atom_p[j])
					< bondLengthLimit(*atom_p[i], *atom_p[j])
...
void Protein::writeHydrophobicity(char *file)  // generate hydrophobicity
	...
	try {
		ofstream Fout(file);
		if (!Fout)
			throw strcat("ERROR: cannot open hydrophobicity file ", file);
		...
		for (i = 0; i < chain.size(); i++) {
			for (j = 0; j < chain[i].residue.size(); j++)
				Fout << chain[i].residue[j].No << " "
						<< chain[i].residue[j].name << " "
...
//read information of torsion angle etc. from DSSP file
	...
	try {
		...
		if (!Fin)
			throw strcat("Cannot open dssp file ", dsspFile);
...
// read alignment from HSSP file
	...
	try {
		...
		if (!Fin)
			throw strcat("Cannot open hssp file ", hsspFile);
