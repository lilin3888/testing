...
void setout(){
    ...
    SGrid <int> cTest1,cTest2,cTest3;

    //cTest1 = coord(1.0,2.0,3.0);
    cTest1.nX=1;
    cTest1.nY=1;
    cTest1.nZ=1;
            ...
            for(ix = -lim;ix <= lim; ix++){
                for(iy = -lim;iy <= lim; iy++){
                    for(iz = -lim;iz <= lim; iz++){
                        ...
                        dist=real(optDot(idist,idist));
                        //ddist = dist + 0.25 + float(idist);
                        ddist.nX = dist + 0.25 + float(idist.nX);
                        ddist.nY = dist + 0.25 + float(idist.nY);
                        ddist.nZ = dist + 0.25 + float(idist.nZ);
